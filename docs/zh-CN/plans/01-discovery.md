# Plan 1 — Discovery 确定性核心 Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** 建立一个零 LLM、纯确定性的 Node CLI，扫描 repo 产出 `inventory.json`（文件清单 + 模态/类别 + git 时间 + 内容 hash + 引用图 + 合成配置），供后续 reviewer/consolidate/report 阶段消费。

**Architecture:** TypeScript（ESM, Node 20）。以 `git ls-files --cached --others --exclude-standard` 列文件（天然遵守 `.gitignore`，无需自行重实现）；各职责拆成小模块（config / classify / scan / hash / gitmeta / refgraph），由 `discovery.ts` 编排成 `Inventory` 并输出 JSON。

**Tech Stack:** TypeScript, Node 20, vitest, `yaml`（解析 audit.yml）。git CLI 为运行期依赖（GH runner 内置）。

对应 spec：`../design.md` §1[1] Discovery、§2 rubric 来源探索、§5 audit.yml、§7 模态分流。
...
每个文件单一职责、可独立测试。`discovery.ts` 只做编排，不含分类/扫描逻辑。
...
Expected: vitest 启动，回报 `No test files found`（尚无测试），exit 0。
...
### Task 1: 共用类型 `src/types.ts`

**Files:**
- Create: `src/types.ts`

无执行逻辑，只定义契约。验证方式为 `tsc --noEmit` 通过。
...
### Task 2: 配置合成 `src/config.ts`

**Files:**
- Create: `src/config.ts`, `test/config.test.ts`

默认：6 个 reviewer 开启、`i18n` 关闭；`report.issueLabel='audit'`、`minSeverity='low'`。audit.yml 缺文件→全默认；部分覆盖→深层合并。
...
### Task 3: 分类 `src/classify.ts`

**Files:**
- Create: `src/classify.ts`, `test/classify.test.ts`

输入：相对路径 + 内容 Buffer。输出：`{ modality, category }`。binary 检测用前 8000 bytes 是否含 NUL；lockfile 用文件名。
...
### Task 4: 列文件 `src/scan.ts`

**Files:**
- Create: `src/scan.ts`, `test/scan.test.ts`

用 `git ls-files --cached --others --exclude-standard` 列文件（天然遵守 .gitignore），返回相对 POSIX 路径数组。lockfile 由文件名集合判定（供上层标记，不在此排除）。

- [ ] **Step 1: 写失败测试**（用 temp git repo）
...
### Task 6: git 元数据 `src/gitmeta.ts`

**Files:**
- Create: `src/gitmeta.ts`, `test/gitmeta.test.ts`

返回 `Map<path, lastCommitISO>`；未跟踪/无记录的文件给 `null`。用单次 `git log` 遍历所有路径的最后 commit 时间（committer date, ISO 8601）。
...
> 注：逐文件 `git log` 在大型 repo 较慢；Plan 4 e2e 若观察到瓶颈再改批量 `git log --name-only` 单次遍历优化。先求正确。
...
### Task 7: 引用图 `src/refgraph.ts`

**Files:**
- Create: `src/refgraph.ts`, `test/refgraph.test.ts`

heuristic：对每个文件，找出哪些"文字类"文件的正文提及它的 basename。返回 `Map<path, referencedBy[]>`。供孤儿检测（`referencedBy` 为空 = 疑似孤儿，由 reviewer 再判）。
...
### Task 8: 编排与 CLI `src/discovery.ts`

**Files:**
- Create: `src/discovery.ts`, `test/discovery.test.ts`

组合前述模块 → `Inventory`。探索规范来源（CLAUDE.md、.claude/skills/**、.claude/workflows/**、.github/workflows/**、.claude/audit.yml）。CLI：`node src/discovery.ts <repoRoot> [outPath]`，输出 JSON（默认 stdout）。

- [ ] **Step 1: 写失败测试**（端到端 temp repo）
...
- `npx vitest run` 全绿（config/classify/scan/hash/gitmeta/refgraph/discovery）
- `node --import tsx src/discovery.ts <repo>` 对任意 git repo 产出合法 `Inventory` JSON
- `inventory.json` 字段齐全（modality/category/hash/lastCommitISO/referencedBy/oversizedText + config + conventions）
- 零 LLM、零网络

## 衔接下一步

`Inventory` 类型即 Plan 2 reviewer 的输入契约。Plan 2 将定义 `findings.json` schema + validator，并以此 inventory 喂给 claude-code-action 内的 lead/reviewer。
