...
**Goal:** synthesis 出力契約（型・validator・prompt アセット）と**決定論的 report pipeline** を構築します：consolidate（`findings/*.json` + synthesis のマージ → 重複除去・ソート済みレポートモデル）、self-contained HTML レポート、GitHub issue markdown、およびファイル読み込みで組み立てる CLI——すべて CI-safe・ゼロ API です。

**Architecture:** TS/ESM/vitest を継続使用します。追加するのはすべて**決定論的 TS + テキストアセット**です：`synthesis.ts`（validator）、`consolidate.ts`（純粋関数によるマージ）、`report-html.ts` / `report-issue.ts`（純粋 render）、`report.ts`（ファイル読み込み CLI）、`reviewers/_synthesis-prompt.md`。実際に LLM を呼び出す synthesis ステップ・gh issue upsert・artifact アップロード・matrix 配線は Plan 4 の範囲です。

**Tech Stack:** TypeScript, Node 20, vitest（継続）。新しい runtime 依存はありません。

対応 spec：`../design.md` §1[3][4][5]、§4（重複除去・ソートルール）、§4.1（synthesis 契約）、§8（synthesis フォールバック）。

### スコープ境界（本 plan では実施しない）
- `claude-code-action` を呼び出しません（synthesis の LLM 実呼び出し → Plan 4）
- GitHub issue への投稿（gh upsert）・artifact アップロードは行いません → Plan 4
- matrix workflow には接続しません → Plan 4
- HTML には「severity によるフィルタ + 折りたたみ」の最小限のインタラクションのみ実装します；ソート・検索などの高度な UI は実装しません（YAGNI）
...
`types.ts` は「契約のみ格納」を維持します；render・マージ・検証ロジックはそれぞれ別ファイルに分離し、単一責任を保ちます。

---

### Task 0: 契約型 `src/types.ts`（追加）

**Files:**
- Modify: `src/types.ts`（ファイル末尾に追記；既存の型は変更しない）

- [ ] **Step 1: 型の追記**（`ReviewerOutput` の後ろに追加）
...
findings と同じスタイルで validator を手書きします。`status:"failed"` の場合は `themes:[]` でなければなりません。
...
- [ ] **Step 3: `reviewers/_synthesis-prompt.md` を作成する**
...
純粋関数です：reviewer outputs をフラット化（failed は `failedReviewers` に記録）、`file|category` で重複除去（代表 = severity×confidence が最高のもの、同率は先着優先）、ソート、stats 計算、synthesis 適用（status が ok の場合のみ使用）。
...
純粋関数で GitHub issue markdown を生成します：安定した upsert マーカー・severity 統計表・exec summary・themes・findings 表・failed reviewers の警告を含みます。
...
### Task 5: HTML レポート `src/report-html.ts`

**Files:**
- Create: `src/report-html.ts`, `test/report-html.test.ts`

純粋関数で self-contained 単一ファイル HTML を生成します：inline CSS/JS・severity カラーコード・severity によるフィルタリング。すべての動的テキストは `esc()` でエスケープします。
...
`findings/` ディレクトリ内の全 `*.json` とオプションの `synthesis.json` を読み込み、consolidate 後に `report.html` と `issue.md` を書き出します。テスト用に loader をエクスポートします。

- [ ] **Step 1: 失敗テストを書く（統合：temp dir fixtures → pipeline）**
...
- `npx vitest run` が全件グリーン（synthesis/consolidate/report-issue/report-html/report/synthesis-assets + 既存テスト）
- `validateSynthesisOutput` が正しく受理・拒否できること（failed-with-themes・不正 priority・related_files の非文字列・semantic_duplicates の形状を含む）
- `consolidate` が正しく重複除去（file+category、代表は最高 severity×confidence を選択）・ソート・stats・synthesis フォールバック（ok/absent/failed）を処理できること
- `renderHtml` が self-contained 単一ファイルを生成できること（外部リソースなし・インジェクション対策のエスケープ済み）・severity フィルタを含むこと
- `renderIssueMarkdown` が upsert マーカー・統計・themes・findings（セル内 pipe のエスケープ）・failed reviewers の警告を含むこと
- `report.ts` CLI が `findings/` + `synthesis.json` から `report.html` + `issue.md` を生成できること
- ゼロ API・ゼロネットワーク

## 次のステップへの橋渡し

Plan 4（最終ピース）：`action.yml` composite + GHA matrix（各 reviewer に 1 つの `claude-code-action` ステップ、Plan 2 の `composeRubric` + `_reviewer-prompt.md` を使用）→ synthesis ステップ（`_synthesis-prompt.md` → `synthesis.json`）→ `report.ts` → gh issue upsert（`ISSUE_MARKER` で既存 issue を検索）+ artifact アップロード；残り 6 名の reviewer の組み込み rubric と `audit.yml` の `paths` glob を補完；`wei18/Sudoku` に対して実際の e2e を実行し、`claude_args` が `--agents`/`Agent` を許可するかどうかを初めてライブ検証します。
