# Plan 1 — Discovery 確定的コア Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** LLM を一切使わない、純粋に確定的な Node CLI を構築します。リポジトリをスキャンして `inventory.json`（ファイル一覧・モダリティ/カテゴリ・git 更新日時・コンテンツハッシュ・参照グラフ・設定合成値）を生成し、後続の reviewer/consolidate/report フェーズへ提供します。

**Architecture:** TypeScript（ESM, Node 20）。`git ls-files --cached --others --exclude-standard` でファイルを列挙します（`.gitignore` を自然に尊重するため、再実装は不要です）。各責務を小さなモジュール（config / classify / scan / hash / gitmeta / refgraph）に分割し、`discovery.ts` が `Inventory` へ編成して JSON を出力します。

**Tech Stack:** TypeScript, Node 20, vitest, `yaml`（audit.yml のパース）。git CLI は実行時依存です（GH runner に内蔵されています）。

対応 spec：`../design.md` §1[1] Discovery、§2 rubric ソース探索、§5 audit.yml、§7 モダリティ分流。
...
test/                       # 與 src 對応する *.test.ts（vitest）
```

各ファイルは単一責務を持ち、独立してテスト可能です。`discovery.ts` は編成のみを担い、分類・スキャンロジックを含みません。

---

### Task 0: プロジェクト scaffold
...
Expected: vitest が起動し、`No test files found` と報告します（テストはまだありません）。exit 0。
...
### Task 1: 共有型定義 `src/types.ts`

**Files:**
- Create: `src/types.ts`

実行ロジックを持たず、契約の定義のみです。検証方法は `tsc --noEmit` の通過です。
...
デフォルト：6 つの reviewer が有効、`i18n` は無効。`report.issueLabel='audit'`、`minSeverity='low'`。audit.yml が存在しない場合はすべてデフォルト値を使用します。部分的な上書きはディープマージで処理します。
...
Expected: FAIL（`config.js` はまだ実装されていません）。
...
入力：相対パス + コンテンツの Buffer。出力：`{ modality, category }`。バイナリ判定は先頭 8000 バイトに NUL が含まれるかで行います。ロックファイルはファイル名で判定します。
...
### Task 4: ファイル列挙 `src/scan.ts`

**Files:**
- Create: `src/scan.ts`, `test/scan.test.ts`

`git ls-files --cached --others --exclude-standard` でファイルを列挙します（`.gitignore` を自然に尊重します）。相対 POSIX パスの配列を返します。ロックファイルはファイル名のセットで判定します（上位レイヤーでのマーキング用であり、ここでは除外しません）。

- [ ] **Step 1: 失敗テストを書く**（一時 git リポジトリを使用）
...
### Task 6: git メタデータ `src/gitmeta.ts`

**Files:**
- Create: `src/gitmeta.ts`, `test/gitmeta.test.ts`

`Map<path, lastCommitISO>` を返します。未追跡・記録なしのファイルには `null` を返します。単一の `git log` で全パスの最終コミット日時（committer date, ISO 8601）を取得します。
...
// 対每个路径查最後 commit 的 committer ISO 時間；無記錄回 null。
...
> 注：ファイルごとに `git log` を呼び出すと大規模リポジトリでは遅くなります。Plan 4 の e2e でボトルネックが確認された場合は、`git log --name-only` の単一走査によるバッチ処理へ改善します。まず正確さを優先します。
...
### Task 7: 参照グラフ `src/refgraph.ts`

**Files:**
- Create: `src/refgraph.ts`, `test/refgraph.test.ts`

ヒューリスティック：各ファイルについて、どの「テキスト系」ファイルの本文にそのファイルの basename が含まれるかを調べます。`Map<path, referencedBy[]>` を返します。孤立ファイル検出に使用します（`referencedBy` が空であれば孤立候補と見なし、reviewer が最終判断します）。
...
### Task 8: 編成と CLI `src/discovery.ts`

**Files:**
- Create: `src/discovery.ts`, `test/discovery.test.ts`

前述のモジュールを組み合わせて `Inventory` を生成します。規約ソースを探索します（CLAUDE.md、.claude/skills/**、.claude/workflows/**、.github/workflows/**、.claude/audit.yml）。CLI：`node src/discovery.ts <repoRoot> [outPath]`、JSON を出力します（デフォルトは stdout）。

- [ ] **Step 1: 失敗テストを書く**（エンドツーエンドの一時リポジトリを使用）
...
- `npx vitest run` が全て緑（config/classify/scan/hash/gitmeta/refgraph/discovery）
- `node --import tsx src/discovery.ts <repo>` が任意の git リポジトリに対して有効な `Inventory` JSON を生成する
- `inventory.json` のフィールドが揃っている（modality/category/hash/lastCommitISO/referencedBy/oversizedText + config + conventions）
- LLM ゼロ、ネットワークゼロ

## 次のステップへの接続

`Inventory` 型が Plan 2 reviewer の入力契約となります。Plan 2 では `findings.json` スキーマ + バリデーターを定義し、この inventory を claude-code-action 内の lead/reviewer へ渡します。
