...
Synthesis step 讀全部 `findings/*.json` + inventory，輸出 `synthesis.json`：
  ...
  "themes": [                         // 跨 reviewer 的系統性主題
    {
      ...
      "finding_refs": [/* 指向相關 findings 的索引/鍵 */],
      "priority": "high | medium | low"
    }
  ],
  "semantic_duplicates": [[/* 語意上重複、應合併的 finding 鍵群 */]],
  "executive_summary": "整體健康度的一段話摘要",
  "status": "ok"                      // synthesis 失敗→report 仍出 raw findings
}
```

Report 同時用 raw findings 與 synthesis；synthesis 失敗時降級為只呈現 raw findings。
